...
import { CATEGORY_EXPORT_SCRIPT } from "@/lib/category-export-script";
import type { Dictionary } from "@/i18n/dictionaries/en";

export function CategoriesImportGuideModal({
  open,
  dict,
  onClose,
}: {
  ...
  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(CATEGORY_EXPORT_SCRIPT);
      setCopied(true);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-end justify-center p-0 sm:items-center sm:p-6">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="categories-import-guide-title"
      ...
      >
        <div className="shrink-0 border-b bg-[var(--surface)] px-5 pb-4 pt-5 sm:px-6">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h2
                id="categories-import-guide-title"
                className="text-lg font-semibold tracking-tight"
              >
                {dict.superAdmin.categoriesImportGuideTitle}
              </h2>
              <p className="mt-0.5 text-sm leading-snug text-[var(--text-muted)]">
                {dict.superAdmin.categoriesImportGuideIntro}
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="space-y-5 overflow-y-auto px-5 py-4 sm:px-6">
          <GuideStep
            number={1}
            title={dict.superAdmin.categoriesImportGuideStep1Title}
            body={dict.superAdmin.categoriesImportGuideStep1Body}
          >
            <VisittScreenshot />
          </GuideStep>
          ...
          <GuideStep
            number={2}
            title={dict.superAdmin.categoriesImportGuideStep2Title}
            body={dict.superAdmin.categoriesImportGuideStep2Body}
          />

          <GuideStep
            number={3}
            title={dict.superAdmin.categoriesImportGuideStep3Title}
            body={dict.superAdmin.categoriesImportGuideStep3Body}
          >
          ...
          <GuideStep
            number={4}
            title={dict.superAdmin.categoriesImportGuideStep4Title}
            body={dict.superAdmin.categoriesImportGuideStep4Body}
          >
            <div className="space-y-2">
              <div className="flex justify-end">
                ...
                >
                  {copied ? dict.common.copied : dict.superAdmin.categoriesImportGuideCopyScript}
                </button>
              </div>
              <pre className="max-h-64 overflow-auto rounded-xl bg-[var(--surface-muted)] p-3 text-xs leading-relaxed">
                <code dir="ltr" className="block text-start font-mono">
                  {CATEGORY_EXPORT_SCRIPT}
                </code>
              </pre>
              <p className="text-xs text-[var(--text-muted)]">
                {dict.superAdmin.categoriesImportGuideScriptNote}
              </p>
            </div>
          </GuideStep>
        </div>
...
function VisittScreenshot() {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="עמוד קריאות ב-Visitt, עם הכפתור פתח קריאה מסומן בפינה השמאלית העליונה"
    >
      ...
      {tableRows.map((row, i) => (
        <g key={row.y}>
          ...
          <circle cx="470" cy={row.y} r="5" fill={row.status} />
          <rect x="240" y={row.y - 6} width="140" height="8" rx="2" fill="#e5e7eb" />
          <rect x="130" y={row.y - 6} width="90" height="8" rx="2" fill="#eef2ff" />
      ...
      <rect x="632" y="128" width="66" height="20" fill="#f4e9ef" />
      <text x="695" y="142" textAnchor="end" fontSize="8" fontWeight="700" fill="#3f3f46">
        קריאות
      </text>
      {[164, 180, 208, 224, 240].map((y) => (
